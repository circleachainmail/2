...
;(function($){
	jQuery.fn.documentor=function(args){
		...
		if(options.indexformat == '1') {
			var countercss = '';
			countercss = "#"+documentHandle+" .doc-menu ol li:before {content: counters(item, \".\") \".\";counter-increment: item;"+options.secstyle; 
			if( options.skin == 'bar' ) {
				countercss = "#"+documentHandle+" .doc-menu ol li a:before {content: counters(item, \".\") \".\";counter-increment: item;"+options.secstyle; 
			}
			jQuery("head").append("<style type=\"text/css\"> #"+documentHandle+" .doc-menu ol {counter-reset: item ;}#"+documentHandle+" ol.doc-menu {margin-top: 20px;}#"+documentHandle+" .doc-menu ol li {display: block;}"+countercss+"}</style>");
		} else {
				...
				jQuery.post(DocAjax.docajaxurl, data, function(response) {
					if( response != "0" ) {
						...
						if( options.scrolling == 1 ) {
							...
							targetid = '#'+targetid; 
							jQuery('html,body').docuScrollTo( targetid, targetid ); 
						}
